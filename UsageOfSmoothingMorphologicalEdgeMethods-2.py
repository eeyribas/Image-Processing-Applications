...
plt.subplot(4,1,2),plt.imshow(blur),plt.title('Blur (Smoothing F.)')
plt.xticks([]), plt.yticks([])
plt.subplot(4,1,3),plt.imshow(dilation),plt.title('Erosion-Opening-Dilation (Morphological F.)')
plt.xticks([]), plt.yticks([])
plt.subplot(4,1,4),plt.imshow(sobel),plt.title('Sobel X-Y (Edge Detection F.)')
plt.xticks([]), plt.yticks([])
